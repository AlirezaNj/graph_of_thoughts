...
#     """
#     Gemini language model interface for Google Gemini 2.5 Flash.
#     Compatible with the Graph of Thoughts framework.
#     """
...
#         self.api_key: str = os.getenv("GEMINI_API_KEY", self.config["api_key"])
#         if self.api_key == "":
#             raise ValueError("GEMINI_API_KEY is not set")
#         genai.configure(api_key=self.api_key, http_options={"base_url": self.base_url})
...
#         if self.cache:
#             self.response_cache[query] = responses if num_responses > 1 else responses[0]
#         return responses if num_responses > 1 else responses[0]

#     @backoff.on_exception(backoff.expo, Exception, max_tries=6, max_time=20)
#     def _generate(self, prompt: str) -> Dict:
#         """
#         Single generation call to Gemini API.
#         """
...
#         self.cost = (
#             (self.prompt_tokens / 1000.0) * self.prompt_token_cost +
#             (self.completion_tokens / 1000.0) * self.response_token_cost
#         )

#         self.logger.info(f"[Gemini] cost so far: ${self.cost:.4f}")
#         return response
...
#     ) -> List[str]:
#         """
#         Extract response texts from Gemini response.
#         """
...
#             try:
#                 texts.append(res.candidates[0].content.parts[0].text)
#             except Exception as e:
#                 self.logger.warning(f"[Gemini] Failed to parse response: {e}")
#         return texts
...
class Gemini(AbstractLanguageModel):
    """
    The Gemini class handles interactions with the OpenAI models using the provided configuration.

    Inherits from the AbstractLanguageModel and implements its abstract methods.
    """

    def __init__(
        self, config_path: str = "", model_name: str = "gemini-lite", cache: bool = False
    ) -> None:
        """
        ...
        if num_responses == 1:
            ...
            while num_responses > 0 and total_num_attempts > 0:
                try:
                    ...
                    self.logger.warning(
                        f"Error in Gemini: {e}, trying again with {next_try} samples"
                    )
    ...
    @backoff.on_exception(backoff.expo, OpenAIError, max_time=60, max_tries=6)
