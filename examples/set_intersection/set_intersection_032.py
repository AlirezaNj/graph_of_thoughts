...
class SetIntersectionPrompter(prompter.Prompter):
    ...
    set intersection example for the language models.ن
...
def got() -> operations.GraphOfOperations:
    ...
    final_aggregate = operations.Aggregate(8)
...
def run(
    ...
    Results are written under results/32/.
    """
    ...
    results_dir = os.path.join(os.path.dirname(__file__), "results", "32")
    difficulty = "easy"
...
SET_INTERSECTION_032_MODEL_SAMPLES = {
    "chatgpt4": 5,
    "gemini": 5,
    "llama4-17b": 5,
}
...
def run_per_model_samples(
    ...
    """Run set_intersection (size 32) with fixed random samples per model. Results under results/32/."""
    data_path = os.path.join(os.path.dirname(__file__), "set_intersection_032.csv")
    data = []
    with open(data_path, "r") as f:
        reader = csv.reader(f)
    ...
    n_total = len(data)
    if n_total == 0:
        logging.warning("No data in set_intersection_032.csv")
        return 0.0
    if seed is not None:
        random.seed(seed)
    total_spent = 0.0
    for lm_name, n_samples in SET_INTERSECTION_032_MODEL_SAMPLES.items():
        if budget <= 0.0:
            break
        n = min(n_samples, n_total)
        data_ids = random.sample(range(n_total), n)
        logging.info(f"Running size 32, {lm_name} with {n} samples: {data_ids}")
        try:
