...
# Prefer local graph_of_thoughts (has Llama API mode for Aval AI) over venv's installed package.
import sys
import os
...
def got() -> operations.GraphOfOperations:
    ...
    operations_graph.append_operation(operations.Generate(1, 8))
    operations_graph.append_operation(operations.Score(3, False))
...
def run(
    ...
    for data in selected_data:
        ...
        for method in methods:
            ...
            # Append response_time and difficulty to the result JSON
            try:
                with open(path, "r", encoding="utf-8") as f:
                    output = json.load(f)
                if output and isinstance(output[-1], dict):
                    output[-1]["difficulty"] = "medium"
...
# Model -> number of random samples for run_per_model_samples
DOC_MERGE_MODEL_SAMPLES = {
    # "chatgpt4": 1,
    # "chatgpt4-nano": 5,
    # "gemini": 1,
    "llama4-17b": 5,
...
def run_per_model_samples(
    ...
    """
    Run doc_merge with a fixed number of random samples per model:
    - chatgpt4-mini: 1 random sample
    - chatgpt4: 4 random samples
    - chatgpt4-nano: 5 random samples
    - gemini: 5 random samples
    - llama13b-hf: 5 random samples

    :param budget: Total budget (dollars) shared across all model runs.
    :param methods: List of method functions (e.g. [io, cot, tot, got, got2]).
    ...
    total_spent = 0.0
    for lm_name, n_samples in DOC_MERGE_MODEL_SAMPLES.items():
        if budget <= 0.0:
        ...
        try:
            ...
            if "gated" in str(e).lower() or "401" in str(e) or "unauthorized" in str(e).lower():
                logging.warning(
                    "Skipping %s: gated/restricted model (log in at huggingface.co or set HF_TOKEN). %s",
                    lm_name,
                    e,
                )
            else:
...
if __name__ == "__main__":
    ...
    # Option 1: Run with fixed number of random samples per model (chatgpt4-mini: 1, chatgpt4: 4, chatgpt4-nano: 5, gemini: 5, llama13b-hf: 5)
    spent = run_per_model_samples(budget, approaches, seed=42)

    # Option 2: Run specific model(s) on specific sample indices
    # for model in ["chatgpt4-mini", "chatgpt4"]:
    #     samples = [item for item in range(20, 40)]
