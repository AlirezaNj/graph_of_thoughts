...
def tot2(all_potential_countries) -> operations.GraphOfOperations:
    ...
    operations_graph.append_operation(operations.Generate(1, 6))
    operations_graph.append_operation(
        operations.Score(1, False, partial(num_errors, all_potential_countries))
    )
    keep_best_1 = operations.KeepBestN(1, False)
...
def got4(all_potential_countries) -> operations.GraphOfOperations:
    ...
    for i in range(1, 5):
        ...
        count_sub_text = operations.Generate(1, 8)
...
# Model -> number of random samples for run_per_model_samples (keyword_counting)
KEYWORD_COUNTING_MODEL_SAMPLES = {
    "chatgpt4": 3,       # ChatGPT 4.1
    "chatgpt4-mini": 3,  # ChatGPT 4.1 mini
    "gemini": 3,         # Gemini 2.5 flash
    "llama4-17b": 3,    # Llama 4 17B
}
...
def run_per_model_samples(
    ...
    """
    Run keyword_counting with a fixed number of random samples per model:
    - chatgpt4 (4.1): 5 random samples
    - chatgpt4-mini (4.1 mini): 5 random samples
    - gemini (2.5 flash): 5 random samples
    - llama70b-hf (Llama 70B): 5 random samples

    :param budget: Total budget (dollars) shared across all model runs.
    :param methods: List of method functions (e.g. [io, cot, tot, tot2, got4, got8, gotx]).
    ...
    n_total = len(data)
    if n_total == 0:
        logging.warning("No data in countries.csv")
        return 0.0
    ...
    total_spent = 0.0
    for lm_name, n_samples in KEYWORD_COUNTING_MODEL_SAMPLES.items():
        if budget <= 0.0:
